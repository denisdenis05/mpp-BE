...
public class AddMovieTests
    {
        ...
        [Test]
        public async Task AfterAddGetAllMovies_ShouldReturnAllMoviesPlusOne()
        {
            // Act: Call the method being tested
            await _movieService.AddMovie(new AddMovieDTO(){Director = "TEST", Genre = "TEST", Rating = 1, Title = "TEST", Writer = "TEST", MPA = "TEST"});
            var result = await _movieService.GetAllMovies();

            // Assert: Verify the result matches expectations
            result.Count.Should().Be(4);
            result.Last().Director.Should().Be("TEST");
        }
